import { createModularEngineAction } from "../../src/helpers/utils";

const createModularEngineActionTests = () => {
  describe("\n   createModularAction", () => {
    test("create a modular-engine action with the given type", () => {
      const resultAction = createModularEngineAction("@@test/action_without_payload");
    ...
    test("if getPayload is set, create a modular-engine action with the returned object inside payload", () => {
      const resultAction = createModularEngineAction(
    ...
    test("include match function inside every action creator, to compare modular-engine actions", () => {
      const resultAction = createModularEngineAction("@@test/test_action");
      expect(
        resultAction.match(createModularEngineAction("@@test/test_action"))
      ).toBe(true);
    });
...
export default createModularEngineActionTests;
